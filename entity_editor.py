...
def hash32_to_float(hash32_val):
    ...
    # Pack as uint32, unpack as float
    bytes_val = struct.pack('<I', int(hash32_val))
...
class ScaleInput(QLineEdit):
    ...
    def update_value(self):
        try:
            ...
            print(f"Error updating scale input: {e}")
            self.setText("1.0")
    ...
    def on_text_changed(self, text):
        try:
            if text.strip():
                ...
                hash32_val = float_to_hash32(float_val)
                self.set_value(hash32_val)
                self.changed.emit()
        except ValueError:
            pass
...
class EntityEditorWindow(QDialog):
        ...
        for field_name, display_name, field_type in main_fields:
            ...
            if field_name == "hidAngles":
                if entity.xml_element and entity.xml_element.find(".//field[@name='hidAngles']") is not None:
                    ...
                    row += 1
            # Special handling for hidScale field
            elif field_name == "hidScale":
                if entity.xml_element and entity.xml_element.find(".//field[@name='hidScale']") is not None:
                    if self.add_xml_field_to_layout(layout, row, field_name, display_name, field_type):
                        row += 1
                else:
                    layout.addWidget(QLabel("Scale:", self), row, 0)
                    add_scale_button = QPushButton("Add Scale Field", self)
                    add_scale_button.clicked.connect(self.add_scale_field)
                    layout.addWidget(add_scale_button, row, 1, 1, 2)
                    row += 1
            else:
                if self.add_xml_field_to_layout(layout, row, field_name, display_name, field_type):
                    row += 1
    ...
    def add_scale_field(self):
        """Add hidScale field to entity if it doesn't exist"""
        if not self.current_entity:
        ...
        # Check if hidScale already exists
        existing_scale = self.current_entity.xml_element.find(".//field[@name='hidScale']")
        if existing_scale is not None:
            QMessageBox.information(self, "Add Scale", "Entity already has scale data (hidScale field).")
            return
        
        try:
            # Find the hidPos_precise field to insert after
            pos_precise_field = self.current_entity.xml_element.find(".//field[@name='hidPos_precise']")
            if pos_precise_field is None:
                QMessageBox.warning(self, "Add Scale", "Could not find hidPos_precise field to insert scale after.")
                return
            
            # Find the parent and the next sibling (Components object)
            parent = None
            for element in self.current_entity.xml_element.iter():
                for child in element:
                    if child == pos_precise_field:
                        parent = element
                        break
            ...
            if parent is None:
                QMessageBox.warning(self, "Add Scale", "Could not find parent element for insertion.")
                return
            
            # Get the original tail from hidPos_precise (this will be moved to hidScale)
            original_tail = pos_precise_field.tail
            
            # Create the new hidScale field element
            from xml.etree import ElementTree as ET
            
            scale_field = ET.Element("field")
            scale_field.set("hash", "00C2DD80")
            scale_field.set("name", "hidScale")
            scale_field.set("value-Hash32", "1065353216")  # 1.0 as Hash32
            scale_field.set("type", "BinHex")
            scale_field.text = "0000803F"  # BinHex for 1.0
            scale_field.tail = original_tail  # Preserve the original indentation/newline
            
            # Update hidPos_precise tail to add proper spacing before hidScale
            pos_precise_field.tail = "\n      "
            
            # Insert the scale field after hidPos_precise
            pos_precise_index = list(parent).index(pos_precise_field)
            parent.insert(pos_precise_index + 1, scale_field)
            ...
            QMessageBox.information(self, "Add Scale", 
                                "Scale field (hidScale) added successfully!\n\n"
                                "The field has been set to 1.0 (no scaling).")
            
        except Exception as e:
            error_msg = f"Failed to add scale field: {str(e)}"
            QMessageBox.critical(self, "Add Scale Error", error_msg)
            print(f"Add scale error: {e}")
    ...
    def create_field_input_widget(self, field_elem, field_name):
        ...
        # Special handling for hidScale - display as float but store as Hash32
        ...
        elif value_attr in ["value-Hash32"] or field_name in ["hidSkyOcclusion0", "hidSkyOcclusion1", "hidSkyOcclusion2", "hidSkyOcclusion3", "hidGroundColor", "hidRigidbodyName", "objModel", "hidResourceId"]:
            # Hash32 fields (excluding hidScale which is handled above)
    ...
    def create_scale_field(self, field_elem, value_attr):
        """Create a scale input field that displays float but stores as Hash32"""
        input_field = ScaleInput(
            self,
            lambda: int(field_elem.get(value_attr) or "1065353216"),  # Default to 1.0
    ...
    def update_scale_field(self, field_elem, value_attr, hash32_val):
        """Update scale field with Hash32 value and corresponding BinHex"""
        try:
            # Set the Hash32 value
            field_elem.set(value_attr, str(hash32_val))
            
            # Update BinHex (little-endian uint32)
            binhex = struct.pack('<I', int(hash32_val)).hex().upper()
    ...
    def schedule_auto_save(self):
        ...
        if self.auto_save_enabled:
            ...
            self.status_label.setText("Changes pending, Please wait.")
