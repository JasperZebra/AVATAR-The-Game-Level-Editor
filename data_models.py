...
class WorldSectorManager:
    ...
    @staticmethod
    def scan_worldsectors_folder(worldsectors_path):
        """
        ...
        try:
            if not os.path.exists(worldsectors_path):
                print(f"Worldsectors path does not exist: {worldsectors_path}")
                return sectors
            
            print(f"Scanning worldsectors folder: {worldsectors_path}")
            ...
            for file in all_files:
                ...
                if os.path.isfile(file_path):
                    if file.endswith(".data.fcb"):
                        ...
                        print(f"  Found FCB: {file}")
                    elif file.endswith(".data.fcb.converted.xml"):
                        # Use .converted.xml files directly - NO COPYING
                        data_xml_files.append(file_path)
                        print(f"  Found converted XML: {file}")
            
            print(f"Total: {len(data_fcb_files)} .data.fcb files and {len(data_xml_files)} .converted.xml files")
            ...
            if data_fcb_files or data_xml_files:
                ...
                print(f"Created sector info with {len(data_fcb_files)} FCB and {len(data_xml_files)} converted XML files")
            
        except Exception as e:
            print(f"Error scanning worldsectors folder: {str(e)}")
            import traceback
            traceback.print_exc()
