from PyQt6.QtWidgets import (QMainWindow, QWidget, QApplication, QFileDialog, QVBoxLayout, 
...
class SimplifiedMapEditor(QMainWindow):
    ...
    def reset_entire_editor_state(self):
        ...
        print("COMPREHENSIVE EDITOR RESET - Clearing all previous level data...")
        ...
        try:
            ...
            print("   Resetting map configuration...")
            self.current_map = None
            self.grid_config = GridConfig(
                sector_count_x=16,
                sector_count_y=16,
            ...
            print("   Resetting modification flags...")
            self.entities_modified = False
            self.xml_tree_modified = False
            self.omnis_tree_modified = False
            self.managers_tree_modified = False
    ...
    def load_level_folder(self):
        ...
        try:
            ...
            progress_dialog = QProgressDialog("Searching and converting files...", "Cancel", 0, 100, self)
            progress_dialog.setWindowTitle("Loading Level")
            progress_dialog.setWindowModality(Qt.WindowModality.WindowModal)
            progress_dialog.setMinimumDuration(0)
            progress_dialog.setValue(0)
            
            # Enhanced search for files
            progress_dialog.setLabelText("Searching for level files...")
            ...
            def update_progress(progress):
                percent = int(10 + progress * 40)  # 10% to 50%
            ...
            try:
                ...
                print("File conversion was interrupted. Continuing with existing XML files...")
            except Exception as e:
                print(f"Error during conversion: {str(e)}. Continuing with existing XML files...")
            ...
            progress_dialog.setLabelText("Loading XML files...")
            ...
            progress_dialog.setLabelText("Processing entities...")
            QApplication.processEvents()
            
            # Load mapsdata first (contains most entities)
    ...
    def load_selected_level_from_dialog(self, dialog, level_list):
        ...
        current_item = level_list.currentItem()
        if not current_item:
            QMessageBox.warning(dialog, "No Selection", "Please select a level to load.")
            return
        
        level_info = current_item.data(Qt.ItemDataRole.UserRole)
        dialog.accept()
        
        # Show confirmation for partial levels
        if not level_info['complete']:
            worlds_status = "Available" if level_info['worlds_path'] else "Missing"
            levels_status = "Available" if level_info['levels_path'] else "Missing"
            
            reply = QMessageBox.question(
                self,
                "Load Partial Level?",
                f"This level has partial data:\n\n"
                f"Level: {level_info['name']}\n"
                f"World Data: {worlds_status}\n"
                f"Level Objects: {levels_status}\n\n"
                f"Continue loading with available data?",
                QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No
            )
    ...
    def load_complete_level(self, level_info):
        ...
        # Create progress dialog
        progress_dialog = QProgressDialog("Loading complete level...", "Cancel", 0, 100, self)
        ...
        try:
            ...
            if level_info['worlds_path']:
                progress_dialog.setLabelText("Loading world data (XML files)...")
                progress_dialog.setValue(10)
                QApplication.processEvents()
            ...
            if level_info['levels_path']:
                progress_dialog.setLabelText("Loading level objects (worldsectors)...")
            ...
            progress_dialog.setLabelText("Checking for terrain data...")
    ...
    def _save_and_convert_worldsectors(self):
        ...
        print(f"\nðŸ’¾ Starting improved WorldSectors save and conversion...")
        
        # Step 1: Save all XML files first
        print(f"ðŸ“ Step 1: Saving modified XML files...")
        saved_xml_files = []
        ...
        print(f"\nStep 2: Converting files using improved method...")
    ...
    def save_level(self):
        ...
        try:
            ...
            # CRITICAL: Save all XML files before conversion
            ...
            worldsector_files_converted = self._convert_worldsector_files_fixed()
    ...
    def save_all_xml_files_before_conversion(self):
        ...
        print(f"\nðŸ’¾ STEP 1: Saving all XML files with current entity positions...")
    ...
    def _convert_worldsector_files_fixed(self):
        """Convert WorldSector .converted.xml files back to .data.fcb - FIXED VERSION"""
        print(f"\nSTEP 2: Converting WorldSector XML files to FCB...")
        
        if not hasattr(self, 'worldsectors_trees') or not self.worldsectors_trees:
            print(f"No WorldSector trees loaded")
            return 0
        ...
        if not converted_xml_files:
            print(f"No .converted.xml files found to convert")
            return 0
        
        print(f"Found {len(converted_xml_files)} .converted.xml files to convert")
        ...
        for xml_file in converted_xml_files:
            try:
                ...
                print(f"\nConverting: {os.path.basename(xml_file)} â†’ {os.path.basename(target_fcb)}")
                ...
                if result_path and os.path.exists(result_path):
                    ...
                    print(f"   Conversion successful: {os.path.basename(result_path)} ({fcb_size} bytes)")
                    
                    # If the result is a _new.fcb file, we need to rename it
                    if result_path.endswith('_new.fcb'):
                        print(f"   ðŸ“ Renaming _new.fcb file to replace original...")
                        
                        # Remove original FCB if it exists
                        if os.path.exists(target_fcb):
                            try:
                                os.remove(target_fcb)
                                print(f"   ðŸ—‘ï¸ Removed original FCB")
                            except Exception as e:
                                print(f"   Could not remove original FCB: {e}")
                        
                        # Rename _new.fcb to target name
                        try:
                            os.rename(result_path, target_fcb)
                            print(f"   Renamed to: {os.path.basename(target_fcb)}")
                        except Exception as e:
    ...
    def _convert_worldsector_xml_to_fcb(self):
        ...
        print(f"\nStarting WorldSector XML to FCB conversion...")
        ...
        if cleanup_files:
            print(f"\nðŸ§¹ Cleaning up {len(cleanup_files)} successfully converted XML files...")
        ...
        if worldsectors_path:
            print(f"\nðŸ§¹ Checking for leftover _new.fcb files...")
            try:
    ...
    def save_worldsectors_changes(self):
        ...
        try:
            ...
            print("Step 1: Saving modified .converted.xml files...")
            ...
            print("Step 2: Converting .converted.xml files back to .data.fcb...")
            ...
            if converted_count > 0:
                print("Step 3: Cleaning up .converted.xml files...")
    ...
    def _convert_main_xml_to_fcb(self):
        """Convert main XML files back to FCB format - CORRECTED VERSION"""
        print(f"\nStarting XML to FCB conversion...")
        ...
        if hasattr(self, 'xml_file_path') and self.xml_file_path and os.path.exists(self.xml_file_path):
            ...
            print(f"  ðŸ“„ Found main XML: {os.path.basename(self.xml_file_path)}")
        ...
        for file_type in ['omnis', 'managers', 'sectorsdep']:
            ...
            if file_path and os.path.exists(file_path):
                ...
                print(f"  ðŸ“„ Found {file_type} XML: {os.path.basename(file_path)}")
        
        if not main_xml_files:
            print(f"No main XML files found to convert")
            return 0
        
        print(f"Found {len(main_xml_files)} XML files to convert to FCB")
            ...
            try:
                ...
                if success:
                    if os.path.exists(fcb_file):
                        ...
                        try:
                            os.remove(xml_file)
                            print(f"  ðŸ—‘ï¸ Cleaned up XML file: {os.path.basename(xml_file)}")
                        except Exception as cleanup_error:
                            print(f"  Could not remove XML file: {cleanup_error}")
                    else:
                        print(f"  Conversion reported success but FCB file not created")
                else:
                    print(f"  Conversion failed for: {os.path.basename(xml_file)}")
        ...
        print(f"\nðŸ“Š Conversion summary: {converted_count}/{len(main_xml_files)} files converted to FCB")
        return converted_count
    ...
    def duplicate_selected_entities(self):
        ...
        print(f"\nDUPLICATING {len(selected_entities)} entities...")
    ...
    def delete_selected_entities(self):
        ...
        print(f"\nðŸ—‘ï¸ DELETING {len(selected_entities)} entities...")
                ...
                if hasattr(self, 'worldsectors_trees') and entity.source_file_path in self.worldsectors_trees:
                    ...
                    try:
                        ...
                        if success:
                            ...
                            print(f"  Failed to remove from XML, but continuing...")
    ...
    def check_all_violations(self):
        ...
        try:
            ...
            if not hasattr(self.canvas, 'sector_data') or not self.canvas.sector_data:
                print("Loading sector data for violation check...")
                success = self.load_sector_data_for_canvas()
                if not success:
                    QMessageBox.warning(
                        self,
    ...
    def test_sector_boundaries(self):
        ...
        print("Enabling sector boundaries...")
    ...
    def update_entity_tree_colors_only(self):
        ...
        try:
            ...
            print("Updating entity tree colors...")
    ...
    def reset_view(self):
        ...
        print("Resetting view to show all content...")
                ...
                for xml_file in sector.data_xml_files:
                    try:
                        if xml_file.endswith('.converted.xml'):
                            print(f"Loading objects from: {xml_file}")
                            ...
                            print(f"Skipping non-converted XML file: {xml_file}")
